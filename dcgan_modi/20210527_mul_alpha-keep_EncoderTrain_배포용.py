...
for epoch in range(opt.niter + 1):
    for i, data in enumerate(tqdm.tqdm(dataloader, desc='batch')):
        ...
        # repaint_real과 fake이미지를 fake로 판단
        ...
        #repaint와 fake의 차이를 maximize하는 입실론 선택. 입실론이 증가됨
        maximize_diff = torch.mean(torch.abs(repaint_real - fake))

        output_fake = netD(fake)
        output_repaint_real = netD(repaint_real)

        #D(fake) - D(repaint) 차이를 maximize하는 입실론 선택. 입실론이 감소됨
        maximize_output_fake = torchmean(torch.mean(output_fake) - torch.mean(output_repaint_real)
        ...
        # fake와 repaint를 real이미지로 판단받도록 G 학습
