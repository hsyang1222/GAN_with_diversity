...
쉽게 0.5를 넘어가기 위해서는 repaint를 모방하면 되지만, 1점을 받기 위해서는 fake를 만들어야 함

mul_alpha_to0 = torch.relu(torch.mean(D(repaint) - D(fake))). repaint보다 fake가 더 그럴듯할 경우 


mul_alpha_to1 = torch.mean(fake - repaint) --> maximize. 다양성을 위해서 alpha를 증가시켜야 함.



방법 2
아예 repaint를 don't care함
...
args = easydict.EasyDict({
    ...
    'try_div_chance' : 0,
    'device':'cuda:2',
    'name' : 'keepE+D(repain)+mul+move_focus',
    'report_every' : 10,
    'keep_try_over' : 0.8,
    'mul_alpha_param' : 1
})
...
mul_alpha = torch.tensor([-9.21204], requires_grad=True, device=device)
