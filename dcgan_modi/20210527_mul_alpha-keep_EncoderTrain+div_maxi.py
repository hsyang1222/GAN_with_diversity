...
args = easydict.EasyDict({
    ...
    'try_div_chance' : 0,
    'device':'cuda:1',
    'name' : 'keepE, G(D(noised)), div_distance',
    'report_every' : 10,
    'keep_try_over' : 0.8,
    'mul_alpha_param' : 1
})
...
optimizerM = torch.optim.Adam([mul_alpha], lr=0.01) 
...
for epoch in range(opt.niter + 1):
    for i, data in enumerate(tqdm.tqdm(dataloader, desc='batch')):
        ...
        maximize_output_fake = torch.mean(output_fake)
    ...
    if config.report_every > 0 and epoch % config.report_every == 0:
        ...
        wandb.log({
            ...
            'D(noised)' : maximize_output_fake,
            'mul_alpha' : sigmoid_mul_alpha,
            'distance_real_from_noised' : distance_real_from_noised,
            'distance_rpreal_from_rpnoised' : distance_rpreal_from_rpnoised
        })
