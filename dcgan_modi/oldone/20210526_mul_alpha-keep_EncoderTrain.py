...
mul_alpha = torch.tensor([-3.0], requires_grad=True, device=device)
...
for epoch in range(opt.niter + 1):
    for i, data in enumerate(tqdm.tqdm(dataloader, desc='batch')):
        ...
        maximize_output_fake = torch.mean(output_fake - output_repaint_real)
    ...
    if config.report_every > 0 and epoch % config.report_every == 0:
        ...
        wandb.log({
            ...
            'D(noised) - D(repaint) ' : maximize_output_fake,
