...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        netD.zero_grad()
        z_0dot5 = (z1[1:] + z1[:-1]) / 2
        fake_z0dot5 = netG(z_0dot5)
        predict = netD(fake_z0dot5.detach())
        errD_fake = criterion(predict, label[:-1]) 
        errD_fake.backward()
        optimizerD.step()
        
        D_G_z1 = predict.mean().item()
        ...
        fake_z1 = netG(z1)
        loss_maximize_div = torch.mean(torch.abs(fake_z1[1:] - fake_z1[:-1]))
        predict = netD(fake_z1)
        errG = criterion(predict, label) - config.lambda_diverse * loss_maximize_div
        errG.backward()
        optimizerG.step()
        ...
        netD.zero_grad()
        z_0dot5 = ((z1[1:] + z1[:-1]) / 2).detach()
        fake_z0dot5 = netG(z_0dot5)
        predict = netD(fake_z0dot5)

        diff_fakes_z1z2 = torch.mean(torch.abs(fake_z1[1:] - fake_z1[:-1]), dim=(1,2,3)).detach()
        diff_fake_z1dot5_z1 = torch.mean(torch.abs(fake_z1[1:].detach() - fake_z0dot5), dim=(1,2,3))
        diff_fake_z1dot5_z2 = torch.mean(torch.abs(fake_z1[:-1].detach() - fake_z0dot5), dim=(1,2,3))
        loss_uniform_diff = mse(diff_fake_z1dot5_z1, diff_fakes_z1z2/2) + mse(diff_fake_z1dot5_z2, diff_fakes_z1z2/2)

        errG = criterion(predict, label[:-1]) + config.lambda_uniform * loss_uniform_diff
        errG.backward()
        optimizerG.step()
